...
function renderDepsPanel() {
  ...
  // Helper: récupérer l'icône d'un container depuis inspectData
...
function buildRow(gi, ci) {
  ...
  // Bouton ajouter dépendance manuelle (visible au survol de la row)
  ...
  addDepBtn.addEventListener('click', (function(i, j, cn) {
    return function(e) {
      e.stopPropagation();
      var depName = prompt(t('prompt_dep_name') || ('Depends on: '));
      if (!depName || !depName.trim()) return;
      depName = depName.trim();
      if (!groups[i].containers[j].deps) groups[i].containers[j].deps = [];
      if (groups[i].containers[j].deps.indexOf(depName) < 0) {
        groups[i].containers[j].deps.push(depName);
        groups[i].containers[j].waitFor = true;
        if (!groups[i].containers[j].timeout) groups[i].containers[j].timeout = 30;
      }
      render();
    };
  })(gi, ci, rowCname));
...
function render() {
  ...
  // Suggestion parallèles
  if (typeof suggestParallelGroups === 'function' && classifyDone) {
...
// ── Dep graph modal ──────────────────────────────────────────
// Nettoyage global si drag annulé (Escape, sortie fenêtre, etc.)
document.addEventListener('dragend', function() {
  // Nettoyer les états sales si le drag se termine sans drop valide
  setTimeout(resetDragState, 50);
  document.querySelectorAll('.drag-over-group, .dragging, .dragging-row, .drag-over-row').forEach(function(el) {
    el.classList.remove('drag-over-group', 'dragging', 'dragging-row', 'drag-over-row');
  });
});
