...
function classifyContainers() {
  ...
  setTimeout(function() {
    if (typeof suggestParallelGroups === 'function') suggestParallelGroups();
  }, 0);
}
...
var KNOWN_DEPS = [
  ...
  { image: /navidrome/i,        needs: /jellyfin|plex/i,         type: 'volume', label: 'Navidrome → Media (dossier partage)' },
];
...
function parseInspect(raw) {
  ...
  Object.keys(volMap).forEach(function(src) {
    ...
    if (sharers.length < 2) return;
    // For each pair, create a bidirectional "shares volume" dep (type: volume — pas de dépendance d'ordre)
...
function detectCheckCmd(c, imgName, containerName) {
  ...
  // 0-pre. Si NetworkMode=container:X, les ports sont exposés par le parent VPN
  //        → utiliser les PortBindings du parent pour construire realPortMap
  var _nm = ((c.HostConfig || {}).NetworkMode || '');
  if (_nm.indexOf('container:') === 0) {
    var _rawParent = _nm.replace('container:', '');
    // Résoudre ID court → nom si nécessaire
    var _parentName = (window.containerIdMap && (containerIdMap[_rawParent] || containerIdMap[(_rawParent).substring(0,12)])) || _rawParent;
    // Chercher les données du parent dans inspectData
    var _parentData = null;
    if (window.inspectData) {
      for (var _pi = 0; _pi < inspectData.length; _pi++) {
        if ((inspectData[_pi].Name || '').replace(/^\//, '') === _parentName) {
          _parentData = inspectData[_pi]; break;
        }
      }
    }
    if (_parentData) {
      // Utiliser les PortBindings du parent
      var _parentPB = (_parentData.HostConfig || {}).PortBindings || {};
      Object.keys(_parentPB).forEach(function(binding) {
        var cPort = binding.replace('/tcp','').replace('/udp','');
        var binds = _parentPB[binding];
        if (Array.isArray(binds) && binds.length > 0 && binds[0].HostPort) {
          realPortMap[cPort] = binds[0].HostPort;
        }
      });
      // Compléter avec NetworkSettings.Ports du parent si disponible
      var _parentNP = (_parentData.NetworkSettings || {}).Ports || {};
      Object.keys(_parentNP).forEach(function(binding) {
        var cPort = binding.replace('/tcp','').replace('/udp','');
        var binds = _parentNP[binding];
        if (Array.isArray(binds) && binds.length > 0 && binds[0].HostPort) {
          realPortMap[cPort] = binds[0].HostPort;
        }
      });
  ...
  if (hc.length > 1 && hc[0] !== 'NONE') {
    var hcCmd = hc.slice(1).join(' ');
    // jq n'est pas disponible par défaut sur Unraid → passer aux presets
    if (/\bjq\b/.test(hcCmd)) {
  ...
  if (preset === '__NONE__') {
    // Healthcheck explicitement désactivé pour ce service (ex: script Python sans HTTP)
    return { cmd: '', level: 'none' };
  }
  if (preset) {
...
// ── Commandes de test prédéfinies ─────────────────────────────────────────
// HEALTHCHECK_PRESETS est défini dans udo-healthchecks.js (chargé avant ce fichier)
// La couche communautaire est dans udo-community.js (chargé après ce fichier)
...
function getPresetCmd(imageName, containerName) {
  ...
  // 2. Preset communautaire sélectionné (couche future — udo-community.js)
  // Retourne null tant que la feature n'est pas activée
  if (typeof getCommunityPresetCmd === 'function') {
    var communityCmd = getCommunityPresetCmd(imageName, containerName);
    if (communityCmd) return communityCmd;
  }

  // 3. Correspondance exacte dans HEALTHCHECK_PRESETS intégrés
  // null = healthcheck explicitement désactivé (ex: qbit_manage)
  // 3a. Nom brut (ex: 'mariadb-official', 'nginx-proxy-manager')
  var crawKey = craw.replace(/[^a-z0-9-]/g,'');
  if (crawKey in HEALTHCHECK_PRESETS) {
    if (HEALTHCHECK_PRESETS[crawKey] === null) return '__NONE__';
    if (HEALTHCHECK_PRESETS[crawKey]) return HEALTHCHECK_PRESETS[crawKey];
  }
  // 3b. Nom normalisé sans tirets (ex: 'nginxproxymanager', 'mariadbofficial')
  if (cname in HEALTHCHECK_PRESETS) {
    if (HEALTHCHECK_PRESETS[cname] === null) return '__NONE__';
    if (HEALTHCHECK_PRESETS[cname]) return HEALTHCHECK_PRESETS[cname];
  }
  // 3c. Image normalisée (fallback — moins spécifique car 'nginx' peut matcher npm)
  if (img in HEALTHCHECK_PRESETS) {
    if (HEALTHCHECK_PRESETS[img] === null) return '__NONE__';
    if (HEALTHCHECK_PRESETS[img]) return HEALTHCHECK_PRESETS[img];
  }

  // 4. Correspondance partielle — clés les plus longues en premier
  var keys = Object.keys(HEALTHCHECK_PRESETS).sort(function(a,b){ return b.length - a.length; });
  for (var i = 0; i < keys.length; i++) {
    var ki = keys[i].replace(/[^a-z0-9]/g,'');
    if (!ki) continue;
    if (img.indexOf(ki) >= 0 || cname.indexOf(ki) >= 0) {
      if (HEALTHCHECK_PRESETS[keys[i]] === null) return '__NONE__';
      return HEALTHCHECK_PRESETS[keys[i]];
    }
  }
  return '';
}
...
// typeColor, computeDepLayout, drawDepGraph, renderDepGraph, renderDepGraphModal
// sont définis dans udo-render.js (chargé avant classify.js)
