...
function saveConfig(array $data): array {
  ...
  if (isset($data['settings'])) {
    $forceObj($data['settings'], ['services','timing','pauses']);
  }
...
function installScript(array $data): array {
  ...
  // Fréquence par défaut selon le mode
  $freqMap = ['start' => 'start', 'stop' => 'stop', 'update' => 'custom'];
  $frequency = $freqMap[$mode] ?? 'disabled';
  $customVal = ($mode === 'update') ? '0 4 * * 0' : '';  // dimanche 4h par défaut pour update
  ...
  $cfg2['scripts'][$mode] = ['generatedAt' => date('c'), 'name' => $name];
...
function readLog(): array {
  // Lire le log le plus récent parmi start/stop/update
  $logs = [
    '/tmp/udo_start_order.log',
    '/tmp/udo_stop_order.log',
    '/tmp/udo_update_order.log',
  ];
  // Retourner celui modifié le plus récemment
  $latest = ''; $latestTime = 0;
  foreach ($logs as $f) {
  ...
  if (!$latest) return ['success' => true, 'log' => '', 'file' => ''];
  return ['success' => true, 'log' => file_get_contents($latest), 'file' => basename($latest)];
}
...
function checkScriptsFresh(): array {
  ...
  foreach (['start', 'stop', 'update'] as $mode) {
    ...
    if ($tSaved > strtotime($genAt)) {
      // Config plus récente que le script → obsolète
      $staleScripts[] = $mode;
    }
  }
